...
begin
  ...
  try
    ...
    on E: Exception do
    begin
      MessageDlg('We could not reach the User database!' +
...
begin
  ...
  if ADOQuery1LoginForm.Recordcount = 0 then
    ShowMessage('We could not find the data!')
  else
  begin
    if Administrator = True then
    begin
      MessageDlg('Logged in as Administrator!!!' + sLineBreak +
        'The login was successful!' + sLineBreak + 'Welcome ' +
...
//       frmMainMenu.Edit1.Text := (edtUserName.Text);
    ...
    begin
      ShowMessage('The login was successful!' + sLineBreak + 'Welcome ' +
        edtUserName.Text + '!');
      ADOQuery1LoginForm.Active := False;
      frmMainMenu.lblUserRights.Caption := (edtUserName.Text);
//       frmMainMenu.Edit1.Text := (edtUserName.Text);
